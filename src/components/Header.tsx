    ...
    <header className="w-full shadow-md bg-surface sticky top-0 z-50">
      ...
      <nav aria-label="Main navigation" className="hidden md:block border-t border-black/10 bg-brand">
        <div className="max-w-7xl mx-auto px-4">
          <ul className="flex">
            {navItems.map((item) => {
              const topHref = withTrailingSlash(item.href)
              return (
              <li
                key={item.href}
                className="relative"
                onMouseEnter={() => setOpenTop(topHref)}
              ...
              >
                ...
                {/* Mega menu */}
                {item.columns?.length && openTop === topHref && (
                      ...
                      <div key={`col-${ci}`}>
                        ...
                        {col.links.map((link, li) => (
                          <div key={`link-${li}`} className="mb-3">
                            <Link href={withTrailingSlash(link.href)} className="block text-sm font-semibold text-text-secondary hover:text-brand mb-1">
                              {link.label}
                            </Link>
                            {link.subGroups?.map((sg, si) => (
                              <div key={`sg-${si}`} className="pl-3 mb-2">
                                ...
                                {sg.links.map((sl, sli) => (
                                  <Link key={`sl-${sli}`} href={withTrailingSlash(sl.href)} className="block text-xs text-text-secondary hover:text-brand py-0.5">
                                    {sl.label}
                                  </Link>
                                ))}
                              </div>
              ...
              <div key={item.href} className="border-b border-black/10 last:border-0">
                ...
                {item.columns?.map((col, ci) => (
                  <div key={`mcol-${ci}`} className="pl-4 pb-2">
                    ...
                    {col.links.map((link, li) => (
                      <Link key={`mlink-${li}`} href={withTrailingSlash(link.href)} className="block text-sm text-header/90 hover:underline py-0.5" onClick={() => setMobileOpen(false)}>
                        {link.label}
                      </Link>
                    ))}
                  </div>
