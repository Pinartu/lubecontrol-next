import {pathsMatch, withTrailingSlash} from '@/lib/internalUrl'
 ...
 * Paths are compared with or without trailing slash; emitted hrefs use trailing slash.
 */
export function getBreadcrumbs(
  ...
  for (const top of navItems) {
    if (pathsMatch(top.href, pathname)) {
      crumbs.push({label: top.label, href: withTrailingSlash(top.href)})
      return crumbs
    }
    for (const col of top.columns ?? []) {
      for (const link of col.links ?? []) {
        if (pathsMatch(link.href, pathname)) {
          crumbs.push({label: top.label, href: withTrailingSlash(top.href)})
          crumbs.push({label: link.label, href: withTrailingSlash(link.href)})
          return crumbs
        }
        for (const sg of link.subGroups ?? []) {
          for (const sl of sg.links ?? []) {
            if (pathsMatch(sl.href, pathname)) {
              crumbs.push({label: top.label, href: withTrailingSlash(top.href)})
              crumbs.push({label: link.label, href: withTrailingSlash(link.href)})
              crumbs.push({label: sl.label, href: withTrailingSlash(sl.href)})
              return crumbs
            }
          }
        }
